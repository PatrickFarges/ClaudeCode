# crafting_ui.gd v3.0.0
# Craft Minecraft avec drag & drop
# Glisser les ingredients depuis l'inventaire (bas) vers la grille 3x3
# Detection automatique de la recette correspondante
...
# --- Drag & drop state ---
var _grid_contents: Array = []   # 9 Dicts: {"block_type": X, "count": N} ou {}
var _held_item: Dictionary = {}  # {"block_type": X, "count": N} ou {}
var _matched_recipe: Dictionary = {}
var _available_recipes: Array = []

# --- Inventaire joueur (affichage) ---
var _inv_items: Array = []  # [{"block_type": X, "count": N}, ...] — items possedes
var _inv_page: int = 0
...
var _grid_ui: Array = []        # 9 dicts: {btn, tex, count_lbl}
...
var _inv_ui: Array = []         # 36 dicts: {btn, tex, count_lbl, name_bg, name_lbl}
...
func _build_ui():
	...
	# Texture crafting table MC (croppee)
	...
	# --- Grille craft 3x3 (interactive — drag & drop) ---
	for r in range(3):
		for c in range(3):
			var idx = r * 3 + c
	...
	_output_btn.offset_left = ox
	_output_btn.offset_right = ox + slot_px
	_output_btn.offset_top = oy
	_output_btn.offset_bottom = oy + slot_px
	...
	_output_name_lbl.offset_left = out_cx - 80
	_output_name_lbl.offset_right = out_cx + 80
	_output_name_lbl.offset_top = oy - 18
	_output_name_lbl.offset_bottom = oy - 2
	_output_name_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	add_child(_output_name_lbl)

	# --- Slots inventaire joueur (3x9 + 1x9 = 36) ---
	var positions: Array = []
	for row in range(3):
		for col in range(9):
	...
	# --- Curseur (ajoute en DERNIER pour z-order au-dessus de tout) ---
...
func _make_tex_rect(x: float, y: float, sz: float) -> TextureRect:
	...
	tr.offset_left = x
	tr.offset_right = x + sz
	tr.offset_top = y
	tr.offset_bottom = y + sz
...
func _make_count_label(sx: float, sy: float, slot_px: float) -> Label:
	...
	lbl.offset_left = sx + slot_px - 26 * GUI_SCALE
	lbl.offset_right = sx + slot_px - 2
	lbl.offset_top = sy + slot_px - 14 * GUI_SCALE
	lbl.offset_bottom = sy + slot_px
...
func _make_slot(sx: float, sy: float, slot_px: float, icon_sz: float, pad: float) -> Dictionary:
	...
	btn.offset_left = sx
	btn.offset_right = sx + slot_px
	btn.offset_top = sy
	btn.offset_bottom = sy + slot_px
...
func _make_inv_slot(sx: float, sy: float, slot_px: float, icon_sz: float, pad: float) -> Dictionary:
	var d = _make_slot(sx, sy, slot_px, icon_sz, pad)
	# Fond nom semi-transparent
	var bg = ColorRect.new()
	bg.set_anchors_preset(Control.PRESET_CENTER)
	bg.offset_left = sx + 1
	bg.offset_right = sx + slot_px - 1
	bg.offset_top = sy + 1
	bg.offset_bottom = sy + slot_px - 1
	...
	var lbl = Label.new()
	lbl.set_anchors_preset(Control.PRESET_CENTER)
	lbl.offset_left = sx + 2
	lbl.offset_right = sx + slot_px - 2
	lbl.offset_top = sy + 2
	lbl.offset_bottom = sy + slot_px - 2
...
func _update_pagination():
	var total = max(1, ceili(float(_inv_items.size()) / SLOTS_PER_PAGE))
	if _page_label:
...
# --- Clic sur un slot inventaire (bas) ---
func _on_inv_input(event: InputEvent, index: int):
	if not (event is InputEventMouseButton and event.pressed):
		return
	var item_idx = _inv_page * SLOTS_PER_PAGE + index
	...
	if event.button_index == MOUSE_BUTTON_LEFT:
		...
		if _held_item.is_empty():
			...
			# Deposer l'item tenu dans l'inventaire
			player._add_to_inventory(_held_item["block_type"], _held_item["count"])
			_held_item = {}
			_refresh_all()

# --- Clic sur un slot de la grille 3x3 ---
func _on_grid_input(event: InputEvent, index: int):
	...
	if event.button_index == MOUSE_BUTTON_LEFT:
		...
		if not _held_item.is_empty():
			...
			# Prendre la moitie depuis la grille
			if not cell.is_empty():
...
func _on_output_input(event: InputEvent):
	...
	player._add_to_inventory(_matched_recipe["output_type"], _matched_recipe["output_count"])
	# Son
...
# ============================================================
func _check_recipe():
	# Collecter les totaux par type depuis la grille
	...
	# Trouver la meilleure recette (plus d'ingredients consommes = plus specifique)
	var best: Dictionary = {}
...
func _consume_grid_for_recipe(recipe: Dictionary):
	...
	for bt in required:
		...
		for i in range(9):
			if remaining <= 0:
				break
			if _grid_contents[i].is_empty():
				continue
			if _grid_contents[i]["block_type"] != bt:
				continue
...
func _return_items_to_inventory():
	if not player:
		return
	# Remettre l'item tenu
	if not _held_item.is_empty():
		player._add_to_inventory(_held_item["block_type"], _held_item["count"])
		_held_item = {}
	# Remettre les items de la grille
	for i in range(9):
		if not _grid_contents[i].is_empty():
			player._add_to_inventory(_grid_contents[i]["block_type"], _grid_contents[i]["count"])
			_grid_contents[i] = {}
...
func _on_output_hover():
	if not _matched_recipe.is_empty():
		var name = BlockRegistry.get_block_name(_matched_recipe["output_type"])
		_tooltip_label.text = "Cliquer pour crafter : %s" % name
		_tooltip_label.visible = true
...
# ============================================================
# PROCESS (curseur + tooltip suivent la souris)
# ============================================================
