...
- **`bedrock_entity.gd`** : parseur Minecraft Bedrock `.geo.json` → hiérarchie Godot Node3D. **[WIP — partiellement fonctionnel]** Parse la structure des bones, construit des cubes UV-mappés avec textures Bedrock. **Distinction clé** : `bind_pose_rotation` (rotation mesh uniquement, bakée dans les vertices) vs `rotation` (rotation du bone node, propage aux enfants). Supporte format 1.8.0 et 1.12.0+, héritage de géométrie (mouton laineux), inflate, mirror, cube-level rotation, skip_bones (filtrage équipement cheval). Cache géométrie static var. `SCALE = 1/16`. Material : `CULL_DISABLED` + `TRANSPARENCY_DISABLED` + `TEXTURE_FILTER_NEAREST`. **Problèmes restants** : mouton (tête cachée par la laine inflate), loup (structure atypique sans parent), pattes légèrement décollées du corps sur certains modèles
- **`passive_mob.gd`** : mobs animaux avec modèles Bedrock .geo.json. **6 types** : SHEEP, COW, CHICKEN, PIG, WOLF, HORSE — enum `MobType`, données dans `MOB_DATA` (collision_size, geo_path, geo_id, texture, health, meat_name, meat_count, bone_map, skip_bones, leg_nudge). **Modèles Bedrock** via `BedrockEntity.build_model()` : hiérarchie de Node3D avec bones nommés, cubes UV-mappés depuis textures .png/.tga. **Animation procédurale squelettique** : `LEG_FREQ=8.0`, `LEG_AMP=30.0` — pattes diagonales en opposition, ailes poulet, bob tête, wag queue. Bind rotations sauvegardées au setup, restaurées en idle. **Leg nudge** par mob (0.08-0.12) pour réduire le gap body-legs. Mouton : head nudgé en -Z/+Y pour dépasser la laine. **Combat** : `take_hit(damage, knockback)` avec flash rouge 0.3s, mort → `_drop_loot()` (heal joueur direct + Label3D flottant vert "+N Viande"). **Spawn** : `add_to_group("passive_mobs")` pour détection flèches. **Direction face** : `atan2(-dir.x, -dir.z)` car modèles Bedrock orientés -Z
...
**Phase 3a.2 (fait) :** Combat et mobs — **Arc/flèches** (charge MC, physique gravité+drag, particules trail/impact, dégâts flottants Label3D). **6 types de mobs passifs** avec modèles GLB Sketchfab (mouton, vache, poulet, cochon, loup, cheval), scales calibrés par analyse AABB, Y offsets pieds au sol, animation procédurale douce (bobbing+lean, pattes cochon). **Loot viande** sur mort (heal joueur + label flottant). **Monde peuplé** : spawn 60%/chunk, 2-4 mobs, max 100, types par biome. **Pioches 2× plus rapides** (PICK_BOOST), **cross-tool 1.3×** (tout outil sur tout bloc). **Cactus** densité réduite (~0.7%), acacia augmenté (~2.5%). **NPC combat** : health 20, take_hit avec release POI
