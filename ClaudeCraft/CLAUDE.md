...
- **`scripts/mob_converter.py`** (v3.1.0) : convertisseur geo.json Bedrock → GLB skinné des 65 mobs (`--all`, `--list`). Sources = clone sparse `Mojang/bedrock-samples`. Supporte box UV + per-face UV, rotation/pivot per-cube, héritage géométrie v1.8 cross-fichier, overrides hiérarchie/BPR/offsets par mob
...
| - | v21.8.0 | **Opération Sauvetage des Animaux — mob_converter v3.1.0 + 65 GLB reconvertis** : sources vanilla = clone sparse `Mojang/bedrock-samples` (`/mnt/Raid4Tb/Games/Minecraft-Bedrock-samples/`, plus besoin de l'install Bedrock Windows). 4 fixes convertisseur : (1) per-face UV (dict `{"north": ...}`) — requis pour ghast (10/10 cubes skippés avant) ; (2) rotation/pivot per-cube — chicken, frog, goat, camel, armadillo, bogged, breeze avaient leurs cubes plaqués droits ; (3) héritage géométrie v1.8 `geometry.X:geometry.Y` cross-fichier — la witch (héritant de villager.geo.json) n'avait ni tête ni corps, juste nez + chapeaux ; (4) lecture `texture_width/height` depuis description (format 1.12+). Validation : scan binaire GLB avant/après (witch +192 verts tête retrouvée, zéro régression — vex -24 verts = nouveau modèle vanilla 1.19.3), wireframes comparatifs, réimport Godot headless OK. |
| - | v21.9.0 | **Chantier B — moteur d'animation** (bedrock_anim_player v1.4.0 + bedrock_anim_engine.py v1.4.0 + molang_evaluator v1.1.0) : (1) **FIX MAJEUR** — les anims time-based sous controller ne progressaient jamais (`_evaluate_controllers` recréait les entries avec time=0 chaque frame ; seules les anims à `anim_time_update` comme walk marchaient — c'est pourquoi walk OK mais attaques/poses figées). Temps préservés au rebuild + `play()` manuel survit. (2) Interpolation **catmull-rom** (`lerp_mode`) : spline Hermite tangentes différences finies — camel (99 keyframes catmullrom)/armadillo/sniffer/creaking fluides, alignée Python↔GDScript au millième. (3) **blend_transition** appliqué : cross-fade progressif entre états (creaking, sniffer, allay). (4) `query.all_animations_finished`/`any_animation_finished` exposées (controller creaking). (5) Warning one-shot sur les queries Molang inconnues (`warn_unknown`) au lieu du 0.0 silencieux. (6) Python : coercion `loop` String→bool alignée. Validé : banc de test GDScript 7/7 (temps, cross-fade, catmull aligné 6.250, retour d'état), non-régression walk cow ±40°. |
| - | v21.10.0 | **Chantier C — LAVA + Food system + POI spatial hash** : (1) **Bloc LAVA** (BlockType 98) — lacs de lave au fond des grottes (`chunk_generator` v7.4, y ≤ LAVA_LEVEL=10), rendu via pipeline fluide généralisé (`chunk.gd` `_build_fluid_mesh`, matériau émissif orange séparé), propagation `fluid_flow_manager` v1.2.0 (2× plus lente que l'eau, portée 3 blocs, eau+lave → STONE), bucket lave fonctionnel (remplir/verser, raywalk détecte les 2 fluides), dégâts 2 HP/0.5s au contact + ralentissement. (2) **Food system** — `player.gd` : `hunger` 0-20, drain par activité (sprint 0.10/s), manger restaure 6 pts (plus de heal direct), régen +1 HP/2.5s si faim ≥ 18, famine 1 dégât/4s jusqu'à 1 cœur, reset au respawn ; `hotbar_ui` barre de faim vivante (icônes full/half/empty). (3) **POI spatial hash** — `poi_manager.gd` v2.0.0 : index par chunk + recherche en anneaux croissants au lieu du scan O(n) mondial (validé identique au brute force sur 200 POI). |
...
- **Bedrock : PAS besoin d'installer le jeu** — clone sparse du repo officiel `Mojang/bedrock-samples` dans `/mnt/Raid4Tb/Games/Minecraft-Bedrock-samples/` (fait le 2026-07-02, ~30 Mo : `resource_pack/models` + `resource_pack/textures/entity`). Tous les packs versionnés (`vanilla_1.21.0` etc.) y sont fusionnés dans un seul `resource_pack/`. `mob_converter.py` pointe dessus via `resolve_bedrock_path()`. Pour élargir le sparse (sons, autres textures) : `git -C /mnt/Raid4Tb/Games/Minecraft-Bedrock-samples sparse-checkout add resource_pack/<dossier>`.
