...
- **`chunk_generator.gd`** : génération procédurale threadée (4 workers, Mutex), 11 noises (terrain, 2 warp, continental, erosion, temp, humid, 3 caves, ore, stone_var), arbres par biome, minerais souterrains, structures passe 4. **Terrain MC 1.18 simplifié** : continentalness (freq 0.0008) contrôle côtes/intérieur/montagnes, erosion (freq 0.0015) crée vallées/passes, domain warping (2 Simplex, amplitude 50) pour formes organiques. Hauteur = base + inland_boost + hills×anti_erosion + mountains×continental×anti_erosion. Biomes découplés de la hauteur (altitude >110 = montagne auto). Biomes ~4× plus grands (freq 0.0015 vs ancien 0.006-0.008). **Grottes compactes** : tunnels spaghetti `abs(v1)+abs(v2) < 0.15` (~5% air), grandes salles très rares (`abs(v3) < 0.03`), bedrock solide y=0-7. **Minerais en veines** (indépendant des grottes, calibré Simplex réel) : charbon 6.3% (y<80), fer 5.2% (y<55), cuivre 3.1% (y<50), or rare (y<30), diamant très rare (y<16)
...
| 10 | Fait | v17.0.0 — **Refonte génération terrain MC 1.18**. Système continentalness + erosion inspiré Minecraft 1.18. Continentalness (Simplex freq 0.0008) contrôle côtes/intérieur/montagnes à grande échelle. Erosion (Simplex freq 0.0015) crée vallées et passes dans les montagnes. Domain warping (2 Simplex, amplitude 50 blocs) déforme les coordonnées pour un terrain organique. Hauteur découplée des biomes : `_get_terrain_height(noise, continental, erosion)` remplace `_get_continuous_height(noise, elevation)`. Biomes 4× plus grands (freq 0.0015 vs 0.006-0.008). Altitude >110 = montagne automatique (neige/pierre). Montagnes avec contreforts progressifs au lieu de murs abrupts. 11 noises au total (+ warp1, warp2, continental, erosion, − elevation) |
| 11 | À venir | Système de faim actif, chaînes de production, combat PNJ visuel |
