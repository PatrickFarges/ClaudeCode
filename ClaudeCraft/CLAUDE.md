...
| 10 | Fait | v17.0.0 — **Refonte génération terrain MC 1.18 + presets rendu**. Terrain : système continentalness + erosion inspiré MC 1.18, domain warping (amplitude 50), biomes 4× plus grands, altitude >110 = montagne auto, 11 noises. **5 presets rendu** (F2) : Vanilla, Global Illumination, Cloclo Style, ENB Sombre (golden hour chaud, Filmic, fog ambré), ReShade Epique (cinématique épique, fog bleuté, contraste 1.3). `_reset_env()` neutralise toutes les propriétés avant chaque preset. Fix startup : `main.tscn` Environment neutre (plus de SSAO/fog baked) + warmup 15 frames pour laisser le renderer initialiser SSAO/SDFGI. Fix arbres en grille : hash XOR mixing. Fix spawn sous terrain : téléport surface |
