...
- **`tool_registry.gd`** : registre des outils disponibles (enum `ToolType` : NONE, STONE_AXE, STONE_PICKAXE, STONE_SHOVEL, STONE_HOE, STONE_HAMMER). Chaque outil a un chemin vers son modèle JSON, ses textures, un dictionnaire de multiplicateurs de minage par type de bloc (ex: pioche 2x sur STONE/IRON_ORE), et une durabilité. `get_tool_mesh()` charge le modèle via ItemModelLoader
