...
- **`player.gd`** : contrôle FPS (CharacterBody3D), minage progressif (basé sur hardness, accéléré par les outils), placement de blocs (vérif AABB chevauchement), inventaire 9 slots hotbar + slots outils parallèles (`hotbar_tool_slots`), intégration HandItemRenderer pour le bras FPS
- **`hand_item_renderer.gd`** : rendu du bras et de l'item en main (vue FPS). Attaché comme enfant de Camera3D. Bras BoxMesh couleur peau + cube texturé du bloc actif (textures par face depuis TexturesPack avec tint) ou modèle 3D d'outil. Deux chemins de rendu : `update_held_tool_model(ArrayMesh)` pour JSON Blockbench, `update_held_tool_node(Node3D)` pour GLB/glTF (layer 2 + UNSHADED appliqués récursivement). **Bobbing** : balancement avant/arrière (rotation X ±12°, ±20° au sprint) au rythme de la marche. **Swing** : animation Tween (-30° en X, 0.3s) au minage/placement. Layer de rendu 2 dédié (UNSHADED, pas d'ombres). Cache de matériaux par texture+tint
...
- **`tool_registry.gd`** : registre des outils disponibles (enum `ToolType` : NONE, STONE_AXE, STONE_PICKAXE, STONE_SHOVEL, STONE_HOE, STONE_HAMMER). Chaque outil a un chemin vers son modèle (`.json` Blockbench ou `.glb`/`.gltf`), ses textures, un dictionnaire de multiplicateurs de minage par type de bloc (ex: pioche 2x sur STONE/IRON_ORE), et une durabilité. `get_tool_node()` détecte l'extension et retourne un Node3D prêt à l'emploi (GLB instantié ou JSON→MeshInstance3D). `get_tool_mesh()` pour accès direct ArrayMesh (JSON uniquement)
...
**Assets :** `Audio/` (~334 fichiers OGG/MP3), `BlockPNJ/` et `MiniPNJ/` (modèles 3D FBX/GLB/OBJ, personnages Kenney.nl), `NPC/` (dossier PNJ), `assets/Lobbys/` (assets Minecraft .schem/.mca à convertir), `assets/Weapon/` (modèles JSON Blockbench d'armes/outils : Stone Tools 5 outils + textures 64x64, MoreIronShortAxes, MoreStonkAxe, reWoodenAxe, Flame/Lightning Quiver), `TexturesPack/Aurore Stone/` (pack de textures Minecraft complet, ~10K fichiers, utilisé pour les textures de blocs en main et dans le monde)
