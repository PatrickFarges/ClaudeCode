...
- **`passive_mob.gd`** : mobs animaux avec modèles GLB Sketchfab. **6 types** : SHEEP, COW, CHICKEN, PIG, WOLF, HORSE — enum `MobType`, données dans `MOB_DATA` (collision_size, model_path, model_scale, model_y_offset, health, meat_name, meat_count). **Modèles GLB statiques** (pas de skeleton/AnimationPlayer) : préchargés dans `_model_scenes` (static var), instanciés au spawn. **Scales calibrés** par analyse AABB brute : sheep/cow 0.065, chicken 0.045, pig 0.9 (modèle ~1 unité vs ~20 pour les autres), wolf 0.04, horse 0.055. **Y offsets** calculés (raw min_y × scale) : pieds posés au sol. **Animation procédurale douce** : marche = bobbing vertical (sin×0.06) + balancement latéral (sin×3°) + inclinaison avant (-5°) ; idle = respiration subtile. Pig seul a 4 pattes séparées (Object_14/17/20/23) avec swing ±25°. **Combat** : `take_hit(damage, knockback)` avec flash rouge 0.3s, mort → `_drop_loot()` (heal joueur direct + Label3D flottant vert "+N Viande"). **Spawn** : `add_to_group("passive_mobs")` pour détection flèches
