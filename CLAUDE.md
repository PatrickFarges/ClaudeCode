...
- **`game_config.gd`** : configuration centrale (`class_name GameConfig`). Constante `ACTIVE_PACK` pour switcher de texture pack en une ligne (défaut "Aurore Stone", "Faithful64x64" disponible). Fonctions statiques `get_block_texture_path()`, `get_item_texture_path()`, `get_entity_texture_path()`. Tous les scripts utilisent GameConfig au lieu de chemins hardcodés
...
- **`player.gd`** : contrôle FPS (CharacterBody3D), minage progressif (basé sur hardness, accéléré par les outils), placement de blocs (vérif AABB chevauchement), inventaire 9 slots hotbar + slots outils parallèles (`hotbar_tool_slots`) + slots nourriture parallèles (`hotbar_food_slots`), intégration HandItemRenderer pour le bras FPS. **Gestion hotbar** : `assign_hotbar_slot()` remplace le bloc ET efface l'outil/nourriture du slot. Touches 1-9 autorisées quand inventaire ouvert. **Système de nourriture** : maintenir clic droit pour manger (2s), particules rouges, restaure 4 PV. `_update_hand_display()` utilise `hand_renderer.update_held_item_sprite(tool_type)` pour les outils (flat sprite Minecraft-style)
- **`hand_item_renderer.gd`** : rendu du bras et de l'item en main (vue FPS). Attaché comme enfant de Camera3D. Bras BoxMesh couleur peau masqué quand un item est tenu. **Trois chemins de rendu** : `update_held_item(BlockType)` pour les blocs (cube texturé par face depuis le pack actif), `update_held_item_sprite(ToolType)` pour les outils (flat sprite Minecraft-style, quad texturé en diagonale -10/-45/-20 degrés, texture depuis `item/` du pack), `update_held_tool_node(Node3D)` pour les modèles 3D arbitraires (food GLB). **Bobbing** : balancement avant/arrière (rotation X ±12°, ±20° au sprint). **Swing** : Tween -30° en X. Cache de matériaux par texture+tint. Utilise `GameConfig` pour les chemins
...
- **`tool_registry.gd`** : registre des outils (enum `ToolType` : NONE, STONE_AXE, STONE_PICKAXE, STONE_SHOVEL, STONE_HOE, STONE_HAMMER, STONE_SWORD, IRON_PICKAXE, IRON_SWORD, DIAMOND_AXE, DIAMOND_PICKAXE, DIAMOND_SWORD, NETHERITE_SWORD, BOW, SHIELD). Chaque outil a un `item_texture` (nom du PNG dans le pack actif `textures/item/`), des multiplicateurs de minage par bloc, et une durabilité. **Plus de GLB/JSON** — tout est rendu via flat sprites. `get_item_texture_path(tool_type)` retourne le chemin complet. `get_item_texture_name(tool_type)` retourne le nom de texture. Shield utilise `entity/shield_base_nopattern.png` (cas spécial). Mining speeds inchangés : haches couvrent 6 essences bois + bookshelf/barrel, pioches toutes variantes pierre, pelle clay/podzol/moss, houe feuilles + hay
...
- **`texture_manager.gd`** : gestionnaire de Texture2DArray (80 layers, indices 0-79). Utilise `GameConfig.get_block_texture_path()` pour charger les PNG depuis le pack actif. **Auto-détection de résolution** : détecte la taille des textures (16x16, 32x32, 64x64...) depuis la première image trouvée, redimensionne toutes les textures à cette résolution. Affiche le pack actif et la résolution détectée au démarrage. Couvre toutes les variantes : 6 essences de bois, 7 types de feuilles, 7 variantes de pierre, minerais, naturels, décoratifs
...
- `TexturesPack/Aurore Stone/` : pack actif par défaut (~3520 PNG, 16x16), structure `assets/minecraft/textures/{block,item,entity}/`. Les textures **block/** sont utilisées pour le monde et les cubes en main, les textures **item/** pour les outils/armes en main (flat sprite) et dans la hotbar
- `TexturesPack/Faithful64x64/` : pack 64x64 (à peupler — dossier vide pour l'instant)

**Assets :** `Audio/` (~334 fichiers OGG/MP3, dont `Audio/Forest/` 11 MP3 d'ambiance par heure du jour), `BlockPNJ/` et `MiniPNJ/` (modèles 3D FBX/GLB/OBJ, personnages Kenney.nl), `NPC/` (dossier PNJ), `assets/Lobbys/` (assets Minecraft .schem/.mca à convertir), `assets/Weapon/` [LEGACY — plus utilisé par le système d'outils, conservé pour référence] (modèles JSON Blockbench + GLB Sketchfab), `assets/Deco/` (apple.glb pour la nourriture)
...
**Phase 3a.1 (fait) :** Refonte du système de textures et outils — `GameConfig` centralisé pour switcher de pack en une ligne, auto-détection résolution (16→64px), outils en flat sprites Minecraft-style (plus de GLB/JSON Blockbench), textures réelles dans hotbar et inventaire, nettoyage des assets
