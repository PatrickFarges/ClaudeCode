...
**Phase 3a.1 (fait) :** Refonte du système de textures et outils — `GameConfig` centralisé (preload, aliases textures cross-pack), pack Faithful64x64 actif, auto-détection résolution, outils 3D extrudés pixel par pixel (Minecraft-style, épaisseur visible), UV blocs corrigés, textures réelles dans hotbar et inventaire. **Positionnement et swing MC-authentiques** (référence : source MCP940/MC 1.12 décompilé) : BASE_POSITION calqué sur `ItemRenderer.transformSideFirstPerson`, rotation outils (0,-10,40) et blocs (0,45,0) calquées sur `handheld.json`/`block.json`, swing sinusoïdal multi-axes calqué sur `transformFirstPerson` (sin/sqrt curves), déplacement transient pendant le swing
